...
if __name__ == "__main__":
    ...
    GRF = GaussianRF(2, s-2, alpha=2.5, tau=7, device='cpu')
    ...
    p = np.exp(GRF.sample(num_examples+1))*beta

    pbar = tqdm.tqdm(desc=f"Helmholtz Resolution{s}",total=num_examples,position=0,leave=True)
    ...
    torch.save(train_data,f"./data/helmoltz_train.pt")

    torch.save(test_data,f"./data/helmoltz_test.pt")