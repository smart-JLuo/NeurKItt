...
if __name__ == "__main__":
    ...
    train_num,test_num=7000,1400
    num_examples = train_num + test_num

    GRF = GaussianRF(2, s, alpha=2, tau=3, device='cpu')
    w = np.exp(GRF.sample(num_examples+1))
    p = torch.empty(num_examples+1,s-2,s-2).uniform_(-1,1)

    x=[]
    eig_vecs=[]
    eig_vals=[]

    pbar = tqdm.tqdm(desc=f"Heat Resolution{s}",total=num_examples,position=0,leave=True)
    ...
    for i in range(num_examples):
        ...
        x.append(torch.cat((wi.unsqueeze(2),pi.T.unsqueeze(2)),dim=-1).unsqueeze(0))
    ...
    torch.save(train_data,f"heat_train.pt")

    torch.save(test_data,f"heat_test.pt")